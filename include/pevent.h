...
#define DEFINE_EI(our, parent) bool match(const type_info& t) const { return (typeid(our) == t) ? true : parent::match(t); } string type() const { return (parent::type() + "::" + #our) ; }
...
class event_knot
{
  private:
    ...
    union
    {
      const part_method receiver_method; // The function to call on the receiver
      const void_part_method void_receiver_method;
    };
    
    const type_info& model; // The type the receiver wants to listen to

  public:
    
    // Initializer to bind together an eh and a window
    event_knot(event_participant& s, event_participant& r, part_method m, const type_info& t);
    event_knot(event_participant& s, event_participant& r, void_part_method m, const type_info& t);
    ~event_knot();
    ...
    bool is(const event_participant& s, const event_participant& r, part_method p, const type_info& m) const;
    bool is(const event_participant& s, const event_participant& r, void_part_method p, const type_info& m) const;    
...
/* This is a structure representing an event that has occured to a particular
   ...
   yield a string representing its type (only interesting for child classes).
...
struct event_info
{
  ...
  public:
    ...
    // Returns a string detailing this event's type and ancestry
    virtual string type() const { return "event_info"; } 
  
    // Returns true if we are an instance of 't' or descended from 't'
    virtual bool match(const type_info& t) const 
...
// Saves typing: 
typedef list<event_knot*> knot_list; // List of knot-pointers is a knot_list
typedef list<event_knot*>::iterator knot_iterator; // Iterator of a knot_list
...
class event_participant
{
  ...
  public:
    ...
    void listen(event_participant& sender, part_method func, const type_info& model);
    void listen(event_participant& sender, void_part_method func, const type_info& model);
    ...
    void forget(event_participant& sender, part_method func, const type_info& model);
    void forget(event_participant& sender, void_part_method func, const type_info& model);
